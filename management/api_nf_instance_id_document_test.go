...
func TestDeregisterNFInstance(t *testing.T) {
	...
	go func() {
		...
		srv, err := http2_util.NewServer(":30678", util.NrfLogPath, router)
		if err != nil {
			log.Panic(err.Error())
		}

		err2 := srv.ListenAndServeTLS(util.NrfPemPath, util.NrfKeyPath)
...
func TestUpdateNFInstance(t *testing.T) {
	...
	go func() {
		...
		srv, err := http2_util.NewServer(":30676", util.NrfLogPath, router)
		if err != nil {
			log.Panic(err.Error())
		}
